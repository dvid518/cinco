...
export function render() {
    return `
        <section id="sidebar">
            <button class="glass act" data-filtro="todas">📊 Todas</button>
            <button class="glass" data-filtro="accion">📈 Acciones</button>
            <button class="glass" data-filtro="etf">🪙 ETFs</button>
            <button class="glass" data-filtro="crypto">₿ Crypto</button>
        </section>
...
export function abrirModalCompra() {
    const html = `
        <form id="form-comprar" class="form-movimiento">
            <div class="form-group">
                ...
                <span class="form-hint">Busca por símbolo. Si no existe, se creará automáticamente.</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="date" id="compra-fecha" class="form-input" value="${new Date().toISOString().split("T")[0]}">
            </div>
        </form>
    `
    ...
    abrirModal({
        ...
        onConfirm: async () => {
            ...
            if (!simbolo) { alert("El símbolo del activo es obligatorio"); return false }
            if (!cantidad || cantidad <= 0) { alert("La cantidad debe ser mayor a 0"); return false }
            if (!precio || precio <= 0) { alert("El precio debe ser mayor a 0"); return false }
            if (!cuentaId) { alert("Selecciona una cuenta de origen"); return false }

            try {
                // 1. Buscar o crear activo (ahora por uid)
                let activo = await buscarActivoPorSimbolo(uid, simbolo)
                if (!activo) {
                    const nombre = prompt(`Ingresa el nombre completo para "${simbolo}":`, simbolo)
                    if (!nombre) { alert("El nombre es obligatorio"); return false }
                    const tipo = prompt("Tipo (accion/etf/crypto):", "accion") || "accion"
...
export function abrirModalVenta() {
    const posiciones = posicionesData?.posiciones || []
    if (posiciones.length === 0) {
        alert("No hay posiciones para vender")
        return
    }
    ...
    const html = `
        <form id="form-vender" class="form-movimiento">
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="date" id="venta-fecha" class="form-input" value="${new Date().toISOString().split("T")[0]}">
            </div>
        </form>
    `
    ...
    abrirModal({
        ...
        onConfirm: async () => {
            ...
            if (!posicionId) { alert("Selecciona una posición"); return false }
            if (!cantidad || cantidad <= 0) { alert("La cantidad debe ser mayor a 0"); return false }
            if (!precio || precio <= 0) { alert("El precio debe ser mayor a 0"); return false }
            if (!cuentaId) { alert("Selecciona una cuenta de destino"); return false }

            try {
                const posicion = posiciones.find(p => p.id === posicionId)
                if (!posicion) { alert("Posición no encontrada"); return false }
                if (cantidad > posicion.cantidad) {
                    alert(`No tienes suficientes unidades. Disponibles: ${posicion.cantidad.toFixed(4)}`)
                    return false
                }
